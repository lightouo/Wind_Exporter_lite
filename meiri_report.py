...
def get_picture(app,sheet_,range_:str,picture_name:str):
    ...
    time.sleep(0.1)
    chart_.api[1].Paste()
    chart_.api[1].Export(f"{os.path.dirname(app.books[0].api.Path)}\\output\\pic\\{picture_name}.png")
    app.books[-1].close()
    print("-----图片生成done-----")
...
def meiri_report(date=dt.datetime.now()):
    ...
    try:
        for n,i in enumerate(pic_data):
            get_picture(app1,book.sheets[n+3],i[0],f"{date.strftime('%Y-%m-%d')}{i[1]}")
    except:
        quit_app()
    quit_app()  


def tongcun_rank_report(date=dt.datetime.now()):
    ...
    time.sleep(3)
    try:
        get_picture(app1,book.sheets[0],"A1:E33",f"{date.strftime('%Y-%m-%d')}全市场同存排名")
    except:
        quit_app()
    quit_app()  

def tongcun_report(date=dt.datetime.now()):
    ...
    book.save(f'./output/平安同存收益率{date.strftime("%Y-%m-%d")}.xlsx')
    time.sleep(3)
    try:
        get_picture(app1,book.sheets[0],"A1:F29",f"{date.strftime('%Y-%m-%d')}平安同存收益率")
    except:
        quit_app()
    quit_app()
